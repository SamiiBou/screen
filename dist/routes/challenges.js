...
const Participation_1 = __importDefault(require("../models/Participation"));
...
async function verifyPayment(txId, reference, retries = 12, delay = 5000) {
...
router.post('/migrate-participation-price', async (req, res) => {
    try {
        ...
        const challengesWithoutPrice = await Challenge_1.default.find({
...
router.post('/init-default', async (req, res) => {
    try {
        ...
        const existingChallenges = await Challenge_1.default.countDocuments();
        ...
        for (const challengeData of defaultChallenges) {
            const challenge = new Challenge_1.default(challengeData);
            await challenge.save();
...
router.post('/initiate-participation-payment', auth_1.auth, async (req, res) => {
    try {
        ...
        const completedParticipation = await Participation_1.default.findOne({
            userId: req.user._id,
            challengeId,
        ...
        await Participation_1.default.deleteMany({
            userId: req.user._id,
            challengeId,
            paymentStatus: { $in: ['pending', 'failed'] }
        });
        // Générer un ID de référence unique
        const reference = crypto_1.default.randomUUID().replace(/-/g, '');
        // Créer une nouvelle participation en attente de paiement
        const participation = new Participation_1.default({
            userId: req.user._id,
            challengeId,
...
router.post('/confirm-participation-payment', auth_1.auth, async (req, res) => {
    try {
        ...
        const participation = await Participation_1.default.findOne({
            paymentReference: reference,
            userId: req.user._id,
            paymentStatus: 'pending'
        ...
        if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
            ...
            await Challenge_1.default.findByIdAndUpdate(participation.challengeId._id, {
...
router.get('/pending-payments', auth_1.auth, async (req, res) => {
    try {
        // Vérifier si l'utilisateur est admin (vous pouvez ajuster cette logique selon vos besoins)
        // if (!req.user!.isAdmin) {
        //   return res.status(403).json({ error: 'Admin access required' })
        // }
        const pendingPayments = await Participation_1.default.find({
            paymentStatus: 'pending',
...
router.post('/verify-pending-payment', auth_1.auth, async (req, res) => {
    try {
        ...
        const participation = await Participation_1.default.findById(participationId).populate('challengeId');
        if (!participation) {
...
router.post('/retry-payment-verification', auth_1.auth, async (req, res) => {
    try {
...
router.get('/:challengeId', async (req, res) => {
    try {
        ...
        const challenge = await Challenge_1.default.findById(req.params.challengeId);
        if (!challenge) {
...
router.post('/:challengeId/participate', auth_1.auth, async (req, res) => {
    try {
        ...
        if (challenge.participationPrice > 0) {
            ...
            const paidParticipation = await Participation_1.default.findOne({
                userId,
                challengeId,
                paymentStatus: 'completed'
            });
            ...
            const existingParticipation = await Participation_1.default.findOne({ userId, challengeId });
...
router.get('/:challengeId/can-participate', auth_1.auth, async (req, res) => {
    try {
        const { challengeId } = req.params;
        const userId = req.user._id;
        ...
        if (challenge.participationPrice > 0) {
            ...
            const pendingParticipation = await Participation_1.default.findOne({
                userId,
                challengeId,
                paymentStatus: 'pending',
            ...
            const failedParticipation = await Participation_1.default.findOne({
                userId,
                challengeId,
            ...
            const existingParticipation = await Participation_1.default.findOne({
                userId,
                challengeId
            });
...
async function calculateRankings(challengeId) {
    try {
        ...
        for (let i = 0; i < participations.length; i++) {
            await Participation_1.default.findByIdAndUpdate(participations[i]._id, {
                rank: i + 1
            });
        }
    }
