...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const user = await User_1.default.findById(userId).select('username bestTime totalChallengesPlayed');
        if (!user) {
            return res.status(404).json({ message: 'Utilisateur non trouvé' });
        }
        const userParticipations = await Participation_1.default.find({ userId })
